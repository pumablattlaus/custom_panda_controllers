#include "custom_panda_controllers/custom_cartesian_velocity_controller.h"
...
bool CartesianVelocityCustomController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("CartesianVelocityCustomController: Could not get parameter arm_id");
    return false;
  }
  ...
  if (velocity_cartesian_interface_ == nullptr) {
    ROS_ERROR(
        "CartesianVelocityCustomController: Could not get Cartesian velocity interface from "
        "hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianVelocityCustomController: Exception getting Cartesian handle: " << e.what());
    return false;
  }

  auto state_interface = robot_hardware->get<franka_hw::FrankaStateInterface>();
  if (state_interface == nullptr) {
    ROS_ERROR("CartesianVelocityCustomController: Could not get state interface from hardware");
    return false;
  }

  sub_vel = node_handle.subscribe("cartesian_target_velocities", 20, &CartesianVelocityCustomController::velocity_callback, this, ros::TransportHints().reliable().tcpNoDelay());

  return true;
}

void CartesianVelocityCustomController::starting(const ros::Time& /* time */) {
  elapsed_time_ = ros::Duration(0.0);
}

void CartesianVelocityCustomController::update(const ros::Time& /* time */,
...
void CartesianVelocityCustomController::velocity_callback(custom_panda_controllers::CartesianVelocityTarget cartesian_velocities)
{
...
void CartesianVelocityCustomController::stopping(const ros::Time& /*time*/) {
...
PLUGINLIB_EXPORT_CLASS(custom_panda_controllers::CartesianVelocityCustomController,
